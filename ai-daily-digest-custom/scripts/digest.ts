...
function stripHtml(html: string): string {
  let text = html;
  // 先 decode HTML entities，让编码的标签变成真实标签
  text = text.replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>')
    .replace(/&quot;/g, '"').replace(/&#39;/g, "'").replace(/&nbsp;/g, ' ')
    .replace(/&#(\d+);/g, (_, code) => String.fromCharCode(parseInt(code)));
  // 再 strip 真实 HTML 标签
  text = text.replace(/<[^>]*>/g, '');
  // 再 decode 一次（处理双重编码的情况）
  text = text.replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>')
    .replace(/&quot;/g, '"').replace(/&#39;/g, "'").replace(/&nbsp;/g, ' ');
  // 清理多余空白
...
async function callAnthropic(prompt: string, apiKey: string, system?: string): Promise<string> {
  ...
  try {
    ...
    throw new Error(`Anthropic API returned non-JSON response (${rawBody.length} chars): ${rawBody.slice(0, 200)}`);
  }
...
/**
 * 修复 AI 生成的 JSON 中未转义的双引号。
 * 例如: "summary": "测试"生成 SVG"的效果" → "summary": "测试\"生成 SVG\"的效果"
 */
function repairJsonQuotes(text: string): string {
  ...
  while (i < chars.length) {
    ...
    if (ch === '"') {
      ...
      // 在字符串内遇到 "，判断是结构性引号还是内容引号
      // 结构性引号后面跟: , ] } 或空白+这些
      const rest = text.slice(i + 1).trimStart();
      const nextStructural = rest[0];
      if (nextStructural === ':' || nextStructural === ',' || nextStructural === ']' ||
          nextStructural === '}' || nextStructural === undefined) {
        // 结构性引号，结束字符串
        inString = false;
        result.push(ch);
      } else {
...
function parseJsonResponse<T>(text: string): T {
  ...
  // 去除 markdown code fence（可能出现在开头或被中文文本包裹）
  ...
  // 优先匹配 { "results" 开头的 JSON 对象（允许 { 后有空白）
  const resultsMatch = jsonText.match(/\{\s*"results"\s*:\s*\[[\s\S]*\]\s*\}/);
  if (resultsMatch) {
    const matched = resultsMatch[0];
    try {
      return JSON.parse(matched) as T;
    } catch {
      // AI 可能在字符串值中生成了未转义的双引号，尝试修复
      const repaired = repairJsonQuotes(matched);
    ...
    const promises = batchGroup.map(async (batch) => {
      ...
      while (attempts < 2) {
        try {
          ...
          attempts++;
          if (attempts < 2) {
            console.warn(`[digest] Summary batch parse failed, retrying... (${error instanceof Error ? error.message : String(error)})`);
            continue;
          }
          console.warn(`[digest] Summary batch failed after retry: ${error instanceof Error ? error.message : String(error)}`);
          for (const item of batch) {
            summaries.set(item.index, { titleZh: item.title, summary: stripHtml(item.description).slice(0, 200), reason: '' });
          }
        }
      }
    });
